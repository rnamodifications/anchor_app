...
class baseset:
    def __init__(self, bases_fname, params):
        self.name = np.array([])
        ...
        with open(bases_fname) as csvfile:
            ...
            for row in reader:
                ...
                #self.mass = np.append(self.mass, float(row[6]))
                self.id = np.append(self.id, self.mass.shape[0])

                if len(self.id) == 4 and params.use4nucle:
                    break

        print('Initialized Base Database')
        print([item for item in zip(self.name, self.mass)])
    ...
    def findidbymass(self, m, stringency, start=None, internal=None, end=None):
        found = (self.mass >= m - stringency) & (self.mass <= m + stringency) 
...
class compoundset:
    ...
    """
    linkedmass = []
    def __init__(self, centroids_fname, params):
        ...
        with open(centroids_fname) as csvfile:
            ...
            seq_id = 0
            for row in reader:
                if float(row['Mass']) < params.mass_min or \
                float(row['Mass']) > params.mass_max or \
                float(row['RT']) < params.rt_min or \
                float(row['RT']) > params.rt_max:
                    continue
                row_subset = dict()
                for key in row.keys():
                    ...
                    if key in ['Cpd', 'Mass', 'RT', 'Width', 'Vol %', 'Vol']:
                        row_subset[key] = row[key]

                if row_subset:
                    if not row_subset.__contains__('Cpd'):
                        seq_id += 1
                        row_subset['Cpd'] = seq_id
                    self.compounds.append(row_subset)
